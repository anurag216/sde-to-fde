...
function App() {
  ...
  if (view === 'diagnostic') return <Diagnostic state={state} onUpdate={update} onExit={() => setView('dashboard')} />

  return (
    <main className="shell">
      <header className="hero">
        <div><p className="eyebrow">CAREER ALPHA · 0.2B</p><h1>SDE <span>→</span> FDE</h1><p className="lede">Build the missing engineering depth without pretending your existing experience counts for nothing.</p></div>
        <div className="level-card"><span>Current evidence</span><strong>{completed}/{diagnosticChallenges.length}</strong><small>diagnostic challenges attempted · {evaluations} tutor reviews</small></div>
      </header>
      ...
      <section className="grid two">
        <article className="card mission-card"><p className="eyebrow">YOUR NEXT MOVE</p><h2>{completion === 100 ? 'Diagnostic evidence captured' : 'Establish your engineering baseline'}</h2><p>The tutor now evaluates open-ended reasoning and unlocks help progressively. If no API key is configured, the app falls back transparently to pre-authored hints and a lightweight local rubric.</p><div className="progress"><i style={{ width: `${completion}%` }} /></div><div className="row"><strong>{completion}%</strong><button onClick={() => setView('diagnostic')}>{completed ? 'Continue diagnostic' : 'Start diagnostic'}</button></div></article>
        <article className="card"><p className="eyebrow">TUTOR CONTRACT</p><h2>It should coach, not autocomplete you.</h2><p>Hints unlock one level at a time only after you have an attempt. Semantic feedback separates weak terminology from weak reasoning and stores qualitative evidence instead of inventing mastery scores.</p>{completed > 0 && <button className="ghost" onClick={restart}>Reset local progress</button>}</article>
      </section>
      <section><div className="section-heading"><div><p className="eyebrow">SKILL GRAPH</p><h2>Uneven is expected.</h2></div><p>We replace “unknown” only with evidence-backed mastery as you use the platform.</p></div><div className="skill-grid">{skills.map((skill) => <article className="skill" key={skill.id}><div className="skill-top"><span>{skill.target}</span><b>UNKNOWN</b></div><h3>{skill.name}</h3><p>{skill.description}</p></article>)}</div></section>
    </main>
  )
}
...
function Diagnostic({ state, onUpdate, onExit }: { state: LearnerState; onUpdate: (state: LearnerState) => void; onExit: () => void }) {
  ...
  if (finished) {
    ...
    return <main className="shell diagnostic-shell"><button className="back" onClick={onExit}>← Dashboard</button><section className="card finish"><p className="eyebrow">BASELINE · EVIDENCE CAPTURED</p><h1>Diagnostic attempt complete.</h1><p>You completed all {diagnosticChallenges.length} alpha challenges.</p><div className="score-row"><div><strong>{correct}/{objective.length}</strong><span>objective checks</span></div><div><strong>{passingRuns}</strong><span>passing code runs</span></div><div><strong>{tutorReviews}</strong><span>tutor evaluations</span></div></div><p className="note">The next layer turns this raw evidence into a traceable skill profile and a personalized first four-week roadmap.</p></section></main>
  }
  ...
  async function execute() {
    ...
    onUpdate({ ...state, codeRuns: { ...state.codeRuns, [challenge.id]: [...runs, result] } })
    setRunning(false)
  }
...
function RunResult({ result }: { result: CodeRunResult }) {
  return <div className={`run-result ${result.status}`}><div className="run-summary"><strong>{result.status === 'passed' ? 'All tests passed' : result.status === 'timeout' ? 'Execution timed out' : result.status === 'error' ? 'Could not run' : 'Some tests failed'}</strong><span>{result.durationMs} ms</span></div>{result.error && <pre>{result.error}</pre>}{result.tests.map((test) => <div className="test-row" key={test.name}><span>{test.passed ? '✓' : '×'} {test.name}</span><b>{test.passed ? 'pass' : 'fail'}</b>{!test.passed && test.error && <small>{test.error}</small>}</div>)}</div>
}
